# Terraform and Bash Scripts to Create an Apache Server on AWS
The Terraform and User Data Bash scripts in this repository create an Apache server on AWS with a Certbot SSL/TLS certificate.

The `apache.tfvars` file creates two instances, with sample web pages hosted on ap-ub.example.com and ap-al.example.com. The example.com domain is simply a placeholder, and a functional user-provided domain must be used to successfully deploy the servers, as documented in the [Usage](#Usage) section below.
...
### `apache` Module
Deploys AWS EC2 instances from images listed in datasources.tf, runs an AWS User Data script to install Apache and apply security measures, and uploads public keys to enable access to each instance.
...
The AWS User Data Bash script installs Apache and Certbot, creates a virtual host for the address listed in the `domain_name` variable in `apache.tfvars`, and configures the security-related software packages described below. Please note the applicable distribution-specific notes.
...
### Installation
To install the script, either clone the [apache-aws-terraform](.) repo or download the files to the local host. 
...
+ Apache Module
  ...
  + domain_name: The domain name for the Apache virtual host, which must be a subdomain of the domain managed by the AWS Hosted Zone.
...
terraform plan -var-file="apache.tfvars"
terraform apply -var-file="apache.tfvars"
```
